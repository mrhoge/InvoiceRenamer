...
## はじめに
- ⚠️本ソフトウェアはパブリックベータ版です。
- ファイル削除処理や中身を書き換える処理は実装していませんが、不具合が生じる可能性があります。
...
1. 起動したら、「PDFフォルダを選択」を押下し、領収書が保管されている場所を指定してください
   * フォルダ選択後、PDF一覧の最初のファイルが自動的に開かれます。
...
6. 勘定科目をプルダウンから選択して「追加」を押下すると、勘定科目もファイル名に追記できます。
   * 勘定科目プルダウンを開くたびに`accounts.csv`の内容が自動的に再ロードされます。
...
9. ファイル名変更後のファイルは、選択したPDFフォルダ内にrenamedフォルダを作成し、その中にコピーされています。
   ...
   * メニューの「ファイル」→「リネーム後ファイル保存先を開く」で、renamedフォルダを直接開けます。
10. 勘定科目は`accounts.csv`に行を追加することで選択肢を追加できます。
   * "勘定科目名,よみがな"というフォーマットで、１科目ごとに改行してください。
   * ファイルを編集後、プルダウンを開くだけで変更が反映されます（アプリの再起動不要）。
...
`requirements.txt` は動作確認済みのバージョンに固定しています（`==` 指定）。意図しないバージョンが混入するのを防ぐためで、更新は以下の手順で明示的に行ってください。

1. venv上で対象ライブラリを更新する: `pip install -U <ライブラリ名>`
2. アプリを起動し、PDFプレビュー・OCR解析・リネームが動くことを確認する
3. 問題なければ `pip show <ライブラリ名>` で表示されたバージョンを `requirements.txt` に反映する
...
## フィードバックについて
バグの報告や機能改善の提案は、[GitHub Issues] までお寄せください。

皆様のフィードバックが開発の助けになります。
